...
# load Donor dataset from a DataPrep package as a pandas DataFrame
donor = run('DonorData-1.dprep', dataflow_idx=0, spark=False)
print ('Donor dataset shape: {}'.format(donor.shape))

# load features and labels
#X, Y = donor[['age', 'hoursinlast6hours', 'hoursperweek', 'Hours volunteered in last 6 months', 'occupation', 'relationship', 'hours-per-week', 'inc']].values, donor['donation_range'].values
X, Y = donor[['age','Hours volunteered in last 6 months', 'hours-per-week']].values, donor['donation_range'].values
...
precision, recall, thresholds = precision_recall_curve(Y_test, y_scores[:,1],pos_label='101_to_1000')
